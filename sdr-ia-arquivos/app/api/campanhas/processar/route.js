...
import { processarProximoDaFila, resumoFila, ultimosMotivos } from "@/lib/fila";
...
export async function GET() {
  ...
  if (cliente) {
    const resultado = await processarProximoDaFila(cliente.id);
  ...
  for (const { cliente_id } of clientes) {
    ...
    religados.push({ cliente_id, ...(await processarProximoDaFila(cliente_id)) });
  }
