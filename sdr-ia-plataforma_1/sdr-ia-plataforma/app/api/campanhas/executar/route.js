 ...
 *
 * RETELL_PREFIXO_DISCAGEM (opcional): prefixo exigido pelo tronco SIP,
 * colado na frente de DDD + número. Ex.: 968655 -> 96865551980554326.
 * Trocou de operadora? Basta mudar essa variável na Vercel.
 */

// Monta o número final no formato que o tronco espera
function montarNumeroDestino(local) {
    ...
    try {
      const resposta = await fetch("https://api.retellai.com/v2/create-phone-call", {
        ...
        body: JSON.stringify({
          ...
          to_number: montarNumeroDestino(contato.telefone), // prefixo + DDD + número
  ...
  return NextResponse.json({
    ...
    mensagem:
      falhas.length === 0
        ...
        : `${sucessos} ligações criadas e ${falhas.length} falharam. Confira o prefixo do tronco e os números da planilha.`,
  });
}
