 ...
 * Abaixo de 1min10 não houve conversa de verdade: mesmo que
 * apareça uma palavra-chave solta, não vale entrar na lista de
 * follow-up. A ligação continua visível nas outras abas.
 */
const MS_MINIMO_OPORTUNIDADE = 70000; // 1min10

/*
 * NÍVEL DE PRIORIDADE — quem ligar primeiro.
 * ALTA  → sinal forte (projeto concreto, reunião aceita, perfil sem
 *         dívida) ou conversa longa (mais de 2 minutos) com sinal.
 * BAIXA → sinal fraco: pediu retorno, e-mail, projeto pra frente.
 * null  → não é oportunidade.
 */
export function nivelPrioridade(sinais, duracaoMs = null) {
  if (!ehOportunidade(sinais, duracaoMs)) return null;
  const temSinalForte = sinais.some((s) => s.prioridade === 1);
  const conversaLonga = (duracaoMs ?? 0) >= 120000; // 2 min
  return temSinalForte || conversaLonga ? "alta" : "baixa";
}

/**
 * Uma ligação é oportunidade quando houve conversa de verdade.
 * A régua é 1min10: quem falou isso tudo e não recusou É lead,
 * mesmo que nenhuma palavra-chave tenha aparecido — a etiqueta
 * ajuda a priorizar, mas a conversa longa já vale follow-up.
 */
export function ehOportunidade(sinais, duracaoMs = null) {
  // Menos de 1min10: não aconteceu nada de aproveitável
  if (duracaoMs !== null && duracaoMs < MS_MINIMO_OPORTUNIDADE) return false;
  // URA/caixa postal e áudio ruim não são leads; recusa explícita também não
  if (sinais.some((s) => s.id === "ura" || s.id === "audio_ruim")) return false;
  if (sinais.some((s) => s.id === "sem_interesse")) return false;
  return true;
}
