...
export async function garantirTabelas() {
  ...
  // Resumo pré-reunião gerado pela IA (a "cola" lida antes da chamada)
  await sql`ALTER TABLE eventos ADD COLUMN IF NOT EXISTS briefing TEXT;`;
  // Lembretes de reunião no WhatsApp: véspera (com confirmação) e 45min antes
  await sql`ALTER TABLE eventos ADD COLUMN IF NOT EXISTS lembrete_vespera BOOLEAN NOT NULL DEFAULT FALSE;`;
  await sql`ALTER TABLE eventos ADD COLUMN IF NOT EXISTS lembrete_hora BOOLEAN NOT NULL DEFAULT FALSE;`;
  ...
  // Áudio, imagem e demais anexos: guardamos o link para tocar/exibir na conversa
  await sql`ALTER TABLE mensagens_wa ADD COLUMN IF NOT EXISTS midia_url TEXT;`;
  await sql`ALTER TABLE mensagens_wa ADD COLUMN IF NOT EXISTS midia_tipo TEXT;`;
