 ...
 */
const MINUTOS_ENTRE_TENTATIVAS = Number(process.env.MINUTOS_ENTRE_TENTATIVAS || 0);
...
export async function registrarFalha(contatoId, motivo) {
  const { rows } = await sql`
    UPDATE contatos SET
      ...
      -- Com MINUTOS_ENTRE_TENTATIVAS = 0 (padrão), redisca na hora
      proxima_tentativa = NOW() + make_interval(mins => ${MINUTOS_ENTRE_TENTATIVAS}),
      atualizado_em = NOW()
    WHERE id = ${contatoId}
