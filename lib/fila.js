...
/*
 * Número que não existe não merece 7 chances.
 * "Não atendeu" pode ser a pessoa ocupada — vale insistir.
 * "Número inválido" ou "operadora recusou" não muda com o tempo:
 * insistir só queima dinheiro. Esses esgotam em 2 tentativas.
 */
const LIMITE_FALHA_TECNICA = 2;
const FALHAS_TECNICAS = /invalid_destination|dial_failed|telephony_provider_permission_denied|no_valid_payment|erro_disparo|HTTP 4/i;
...
const CONCORRENCIA_MAXIMA = Math.max(Number(process.env.CONCORRENCIA_MAXIMA || 5), 1);
 ...
 * Padrão 8h às 18h: horário comercial, que é onde há retorno.
 * Ligação à noite para empresa quase não converte.
...
const HORA_FIM = Number(process.env.HORA_FIM_LIGACOES ?? 18);       // última ligação antes das 18h
...
export async function registrarFalha(contatoId, motivo) {
  // Falha técnica esgota mais rápido: o número não vai "melhorar"
  const limite = FALHAS_TECNICAS.test(String(motivo)) ? LIMITE_FALHA_TECNICA : LIMITE_TENTATIVAS;

  const { rows } = await sql`
    UPDATE contatos SET
      tentativas    = tentativas + 1,
      status        = CASE WHEN tentativas + 1 >= ${limite} THEN 'ESGOTADO' ELSE 'PENDENTE' END,
      ultimo_motivo = ${motivo},
      call_id       = NULL,
      -- Espaçamento das rediscagens. Com MINUTOS_ENTRE_TENTATIVAS
      -- definido, vale o valor linear de sempre. Sem definir (0),
      -- entra a escala automática: 30min, 2h, 5h, 1 dia, 1 dia,
      -- 2 dias — quem não atende às 10h de terça pode atender às
      -- 16h de quinta. Mesmo custo, muito mais chance de pegar.
      proxima_tentativa = NOW() + (CASE
        WHEN ${MINUTOS_ENTRE_TENTATIVAS}::int > 0
          THEN ${MINUTOS_ENTRE_TENTATIVAS}::int * (tentativas + 1)
        ELSE (ARRAY[30,120,300,1440,1440,2880,2880])[LEAST(tentativas + 1, 7)]
