...
/*
 * Cancela a reunião futura deste lead. Sem isso, pedido de
 * "desmarca" não tinha ação nenhuma: a IA dizia que cancelou,
 * marcava a nova, e as DUAS ficavam na agenda.
 */
export async function cancelarReuniao({ clienteId, telefone }) {
  if (!telefone) return { sucesso: false, motivo: "Sem telefone para localizar a reunião." };

  const { rows } = await sql`
    DELETE FROM eventos
    WHERE id = (
      SELECT id FROM eventos
      WHERE cliente_id = ${clienteId} AND telefone = ${telefone} AND inicio > NOW()
  ...
  if (!rows[0]) {
    return { sucesso: false, motivo: "Não havia reunião futura marcada para este lead." };
  }

  const d = new Date(rows[0].inicio);
  return {
    sucesso: true,
    confirmacao: `Reunião de ${d.toLocaleDateString("pt-BR", { timeZone: TZ })} às ${d.toLocaleTimeString("pt-BR", { timeZone: TZ, hour: "2-digit", minute: "2-digit" })} cancelada.`,
    instrucao:
      "Reunião REALMENTE removida da agenda. Se o lead quer outro horário, agora sim consulte os horários livres e ofereça.",
  };
}

/*
 * Reunião futura já marcada para este telefone. É o antídoto do
 * "bug do espelho": a IA marcava, consultava de novo, encontrava a
 * PRÓPRIA reunião ocupando o horário e "corrigia" oferecendo outros
 * — desmarcando na cabeça do lead o que estava certo na agenda.
 */
export async function reuniaoJaMarcada(clienteId, telefone) {
  if (!telefone) return null;
  const { rows } = await sql`
    SELECT inicio FROM eventos
  ...
  const d = new Date(rows[0].inicio);
  return {
    dia_semana: d.toLocaleDateString("pt-BR", { timeZone: TZ, weekday: "long" }),
    data_falada: d.toLocaleDateString("pt-BR", { timeZone: TZ, day: "numeric", month: "long" }),
...
export async function agendarReuniao({ clienteId, data, hora, nome, telefone, callId = null }) {
  ...
  return {
    ...
    instrucao:
      "REUNIÃO JÁ MARCADA E GRAVADA NA AGENDA. Não consulte horários de novo nesta conversa — o horário vai aparecer ocupado porque é ESTA reunião. Apenas repita dia e hora ao confirmar com o lead.",
  };
}
