...
/*
 * Primeiro nome apresentável: "SORVETES KELLY LTDA" não é nome de
 * gente — razão social e caixa alta ficam de fora; sobra "Olá!".
 */
export function primeiroNomeApresentavel(nome) {
  const limpo = String(nome || "").trim();
  if (!limpo) return null;
  if (/\b(ltda|eireli|mei|s\/?a|epp|cia|comercio|com[ée]rcio|industria|ind[úu]stria|agricola|agr[íi]cola|sindicato|associa\w*)\b/i.test(limpo)) {
    return null; // razão social
  }
  const primeiro = limpo.split(/\s+/)[0];
  if (!/^[a-zá-úA-ZÁ-Ú]/.test(primeiro)) return null;
  // Caixa alta da planilha vira "Samuel", não "SAMUEL"
  return primeiro.charAt(0).toUpperCase() + primeiro.slice(1).toLowerCase();
}

function textoDeFollowUp(nome) {
  const primeiro = primeiroNomeApresentavel(nome);
  const tratamento = primeiro ? `Olá ${primeiro}!` : "Olá!";
...
export async function enviarTemplate({
  ...
  if (canal === "zapi") {
    /*
     * Quem ATENDEU a ligação recebe uma abertura escrita pela IA,
     * continuando a conversa do telefone (nome falado na ligação,
     * assunto que ficou). Sem ligação atendida ou sem IA, vai o
     * texto fixo — com o nome tratado, nunca razão social.
     */
    let partes = null;
    try {
      const { gerarPrimeiraMensagem } = await import("@/lib/ia");
      const gerada = await gerarPrimeiraMensagem(clienteId, telefone);
      if (gerada) {
        partes = gerada.split(/\|\||\n+/).map((t) => t.trim()).filter(Boolean).slice(0, 3);
      }
    } catch (e) {
      console.error("primeira_mensagem_fallback:", e);
    }
    if (!partes || partes.length === 0) partes = [textoDeFollowUp(nome)];

    await garantirTabelas();
    for (let i = 0; i < partes.length; i++) {
      const { ok, dados } = await enviarPelaZapi(telefone, partes[i], {
        delayMessage: i === 0 ? 3 : 2,
        delayTyping: ritmoDeDigitacao(partes[i]),
      });
      if (!ok) {
        if (i === 0) return { ok: false, motivo: JSON.stringify(dados).slice(0, 300) };
        break; // primeira saiu: registra o que foi e segue
      }
      await sql`
        INSERT INTO mensagens_wa (cliente_id, telefone, direcao, texto)
        VALUES (${clienteId}, ${telefone}, 'out', ${partes[i]});
      `;
    }
